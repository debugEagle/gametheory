...
public class NLBetRound<PlayerId> implements Cloneable {
  ...
  public NLBetRound(@NonNull final List<PlayerData<PlayerId>> playersData,
      @NonNull BetRoundSpec<PlayerId> spec) {
    this.spec = spec;
    seq.clear();
    ...
    for (int i = 0; i < nbPlayers; i++) {
      ...
      checkArgument(bigBlind >= bets[i],
          "Player %s has a bet > bigblind at the beginning of the round", pData.getPlayerId());
  ...
  public void doMove(Move<PlayerId> m) {
    ...
    checkArgument(m.getPlayerId() == playerId, "Wrong player %s for this move, expected %s",
        m.getPlayerId(), playerId);
    int val = m.getValue();
    switch (m.getType()) {
      case BET:
