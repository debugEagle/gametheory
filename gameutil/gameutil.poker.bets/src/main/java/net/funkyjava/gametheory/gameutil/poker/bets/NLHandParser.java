...
public class NLHandParser {
	...
	public static NLHand<Integer> parse(final String handStr, final int nbBetRounds) {
		...
		final BlindsAnteSpec<Integer> blindsSpecs = new BlindsAnteSpec<Integer>(parser.hasAnte, parser.hasBlinds, false,
				parser.sbVal, parser.bbVal, parser.anteVal, Collections.<Integer> emptyList(), parser.sbIndex,
				parser.bbIndex);
		final int firstPlayerIndex = parser.hasBlinds ? (parser.bbIndex + 1) % nbPlayers : 0;
		final BetRoundSpec<Integer> betsSpec = new BetRoundSpec<Integer>(firstPlayerIndex, parser.bbVal);
	...
	private final void parseSettings(final String settingsStr) {
		...
		final Optional<Integer> bbValOpt = ProgramArguments.getStrictlyPositiveIntArgument(args, bbPrefix);
		checkArgument(bbValOpt.isPresent(), "No bb value specified, add " + bbPrefix + "XX to the hand string");
		bbVal = bbValOpt.get();
		...
		final Optional<Integer> anteValOpt = ProgramArguments.getStrictlyPositiveIntArgument(args, antePrefix);
		if (anteValOpt.isPresent()) {
			if (!hasAnte) {
				log.warn("You specified ante value but ante are not enabled, you can do it with " + hasAntePrefix
						+ "=true");
			} else {
				anteVal = anteValOpt.get();
			}
		}
		final Optional<Integer> sbValOpt = ProgramArguments.getStrictlyPositiveIntArgument(args, sbPrefix);
	...
	private final List<NoBetPlayerData<Integer>> parsePlayers(final String str) {
		final List<NoBetPlayerData<Integer>> res = new LinkedList<>();
	...
	private final NoBetPlayerData<Integer> parsePlayer(final String str, final int index) {
