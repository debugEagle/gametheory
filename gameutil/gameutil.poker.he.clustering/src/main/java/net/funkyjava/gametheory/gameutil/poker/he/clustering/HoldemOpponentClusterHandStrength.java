...
/**
 * 
 * Class to build histograms of hand strength versus opponent clustered hole
 * cards (OCHS) as shown here :
...
public class HoldemOpponentClusterHandStrength {
	...
	public static double[][] opponentClusterHandStrength(final CardsGroupsIndexer preflopIndexer,
			final int[] preflopBuckets, final int nbBuckets, final Streets street, final WaughIndexer streetIndexer,
			final Holdem7CardsEvaluator eval) {
		...
		for (int i = 0; i < streetIndexSize; i++) {
			...
			if (nbMissingBoardCards > 0) {
				deck.drawAllGroupsCombinations(new int[] { 2, nbMissingBoardCards }, new CardsGroupsDrawingTask() {
					...
					public boolean doTask(int[][] cardsGroups) {
						...
						for (int j = 0; j < nbMissingBoardCards; j++) {
							...
							oppCardsEval[index] = heroCardsEval[index] = translateToEval.translate(missingBoard[j]);
						}
