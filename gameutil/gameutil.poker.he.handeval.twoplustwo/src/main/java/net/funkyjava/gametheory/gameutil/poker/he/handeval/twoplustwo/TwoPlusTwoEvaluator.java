 ...
 * 
 */
public class TwoPlusTwoEvaluator
		implements HoldemFullEvaluator, Holdem5CardsEvaluator, Holdem6CardsEvaluator, Holdem7CardsEvaluator {
	...
	public int compare7CardsHands(int[] h1, int[] h2, int[] board) {
		...
		return handRanks[handRanks[(b = handRanks[handRanks[handRanks[handRanks[handRanks[53 + board[0]] + board[1]]
				+ board[2]] + board[3]] + board[4]]) + h1[0]] + h1[1]] - handRanks[handRanks[b + h2[0]] + h2[1]];
	...
	public int get7CardsEval(int[] hand) {
		final int[] handRanks = Generator.handRanks;
		return handRanks[handRanks[handRanks[handRanks[handRanks[handRanks[handRanks[53 + hand[0]] + hand[1]] + hand[2]]
				+ hand[3]] + hand[4]] + hand[5]] + hand[6]];
	}

	@Override
	public void get7CardsEvals(int[][] hands, int[] board, int[] dest) {
		final int[] handRanks = Generator.handRanks;
		int b = handRanks[handRanks[handRanks[handRanks[handRanks[53 + board[0]] + board[1]] + board[2]] + board[3]]
				+ board[4]];
	...
	public int compare5CardsHands(int[] h1, int[] h2, int[] board) {
		int b;
		final int[] handRanks = Generator.handRanks;
		return handRanks[handRanks[(b = handRanks[handRanks[handRanks[53 + board[0]] + board[1]] + board[2]]) + h1[0]]
				+ h1[1]] - handRanks[handRanks[b + h2[0]] + h2[1]];
	}

	@Override
	public int get5CardsEval(int[] hand) {
		final int[] handRanks = Generator.handRanks;
		return handRanks[handRanks[handRanks[handRanks[handRanks[53 + hand[0]] + hand[1]] + hand[2]] + hand[3]]
				+ hand[4]];
	}
	...
	public int compare6CardsHands(int[] h1, int[] h2, int[] board) {
		int b;
		final int[] handRanks = Generator.handRanks;
		return handRanks[handRanks[(b = handRanks[handRanks[handRanks[handRanks[53 + board[0]] + board[1]] + board[2]]
				+ board[3]]) + h1[0]] + h1[1]] - handRanks[handRanks[b + h2[0]] + h2[1]];
	}

	@Override
	public int get6CardsEval(int[] hand) {
		final int[] handRanks = Generator.handRanks;
		return handRanks[handRanks[handRanks[handRanks[handRanks[handRanks[53 + hand[0]] + hand[1]] + hand[2]]
	...
	public void get6CardsEvals(int[][] hands, int[] board, int[] dest) {
		final int[] handRanks = Generator.handRanks;
		int b = handRanks[handRanks[handRanks[handRanks[53 + board[0]] + board[1]] + board[2]] + board[3]];
		for (int i = 0; i < hands.length; i++)
			dest[i] = handRanks[handRanks[b + hands[i][0]] + hands[i][1]];
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.funkyjava.cscfrm.game.poker.he.handeval.itf.HoldemEvaluator#
	 * getCardsSpec ()
	 */
	@Override
