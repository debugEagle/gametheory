...
public class HoldemHistograms {

	public enum HoldemHistogramsStreets {
		PREFLOP, FLOP, TURN
	}

	public enum HoldemHistogramsValues {
		HS, EHS, EHS2
	}

	public static double[][] generateHistograms(final HoldemHistogramsStreets street,
			final HoldemHistogramsValues nextStreetValue, final int numberOfBars) {
	...
	private static double[][] generateHistograms(final WaughIndexer streetIndexer, final WaughIndexer nextStreetIndexer,
		...
		for (int streetIdx = 0; streetIdx < streetSize; streetIdx++) {
			final double[] histogram = histograms[streetIdx];
			...
			deck.drawAllGroupsCombinations(new int[] { numberOfCardsToAddForNextStreet }, new CardsGroupsDrawingTask() {
				...
				public boolean doTask(int[][] cardsGroups) {
					...
					histogram[(int) Math.round(nextStreetValues[nextStreetIndex] * (numberOfBars - 1))]++;
					nbHits.increment();
					return true;
				}
			});
