...
public class KohonenClusterer<T extends Clusterable> extends Clusterer<T> {
	...
	public KohonenClusterer(DistanceMeasure distance, LearningFactorFunction learningFactor,
			NeighbourhoodSizeFunction neighbourhoodSize, NetworkProvider networkProvider, int taskSamplesSize,
			int maxTasks, RandomGenerator random, NetworkConvergenceMonitorProvider convergenceMonitorProvider) {
		super(checkNotNull(distance));
		updateAction = new KohonenUpdateAction(distance, checkNotNull(learningFactor), checkNotNull(neighbourhoodSize));
	...
	public List<CentroidCluster<T>> cluster(Collection<T> points)
		...
		for (int i = 0; i < pointsSize; i++) {
			...
			if (cluster == null) {
				clusters.put(identifier, cluster = new CentroidCluster<>(new DoublePoint(neuron.getFeatures())));
			}
