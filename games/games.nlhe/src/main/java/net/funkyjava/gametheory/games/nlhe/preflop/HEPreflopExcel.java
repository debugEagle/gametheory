...
public class HEPreflopExcel {
	...
	public static <T> void createStrategiesSheet(final Map<String, CellStyle> styles, final String sheetName,
			final Workbook wb, final CSCFRMData<NLBetTreeNode<T>, ?> data, final CardsGroupsIndexer holeCardsIndexer,
			final Map<T, String> playersNames) {
		...
		rowIndex++;
		final Map<ActionNode<NLBetTreeNode<T>, ?>, CSCFRMNode[]> allNodes = data.nodesForEachActionNode();
		for (ActionNode<NLBetTreeNode<T>, ?> actionNode : allNodes.keySet()) {
			final CSCFRMNode[] nodes = allNodes.get(actionNode);
			final Map<Move<T>, double[][]> strats = HEPreflopHelper.getMovesStrategies(actionNode, nodes,
					holeCardsIndexer);
	...
	private static <T> int writeMovesStrats(final Sheet sheet, final Map<T, String> playersNames,
			final NLBetTreeNode<T> node, final Map<String, CellStyle> styles, final Map<Move<T>, double[][]> strats,
	...
	private static int writeStrat(final Sheet sheet, final Map<String, CellStyle> styles, final double[][] strat,
		...
		for (int i = 0; i < nbRows; i++) {
			...
			for (int j = 0; j < nbColumns; j++) {
				...
				final ClientAnchor anchor = drawing.createAnchor(0, 0, 0, 0, columnIndex, rowIndex, columnIndex + 1,
						rowIndex + 1);
